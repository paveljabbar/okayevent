...
for color in COLORS:
    ...
    for i in range(1, 9):
        card = f"{i}{color}"
        if card in st.session_state.seen_cards:
            continue
        ...
        with row[i-1]:
            if st.button(colored_label, key=f"deck_add_{card}", disabled=used):
...
for i in range(5):
    for i in range(5):
        if i < len(st.session_state.active_hand):
            ...
            label = f":{text_color_map[col]}[{bold_number_unicode(num)}]"
            with active_cols[i]:
                if st.button(label, key=f"deck_active_remove_{i}_{card}", help=f"{card} entfernen"):
